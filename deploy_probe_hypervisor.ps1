...
Mandatory
Specify the destination vcenter to depoy probes in FQDN format, like "por-intvc1.por-ngcs.lan".
.EXAMPLE
.\deploy_probe_hypervisor.ps1 -probeFile pathdeprobefile -destVcenter por-intvc1.por-ngcs.lan
Execute the script with the path to your CSV file and and Destination Vcenter in FQDN format.
.EXAMPLE
.\deploy_probe_hypervisor.ps1 -probeFile pathdeprobefile -destVcenter -createdns 0 -force 1
Execute the script with the path to your CSV file and Destination Vcenter in FQDN Format ( xxxxx.por-ngcs.lan) and no DNS registry creation.
.LINK 
Online version: https://confluence.united-internet.org/display/~jlobatoalonso/deploy+probe+script
.NOTES
2021 Javier Lobato 
2021/09/22 First Release
2022/02/25 Hypervisor version with no DHCP
#>
...
foreach ($probe in $probeList)
{
    ...
    "gb" {
        ...
        # Get vCenter and connect , in this version the user input as argument
